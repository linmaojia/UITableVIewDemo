...
@interface DEMO10_VC ()<UITableViewDelegate, UITableViewDataSource>
{
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.section == 2){
        ...
        return [MJCollectViewCell getScrollowHeight];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ESWeakSelf;
    if(indexPath.section == 2){
        MJCollectViewCell *cell = [MJCollectViewCell shoppingCarCellWithTableView:tableView];
        cell.model = (MJShoppingCarModel *)_dataArray[indexPath.section];
        cell.indexPath = indexPath;
        cell.CollectionImgClick = ^(NSString *productId){
            NSLog(@"---enen-id = %@",productId);
        };
        return cell;
    }else{
